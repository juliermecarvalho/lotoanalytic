...
import {
  ...
  GenerateQuinaGamesResponse,
  LatestContestResponse,
  LotteryModeResponse
} from "../../lib/apiClient";
import { buildGamesCsv, buildGamesScript } from "./gameExport";
import { loadQuinaPreferences, saveQuinaPreferences } from "./quinaPreferences";
...
// Tela de geracao de jogos da Quina: monta os filtros estatisticos e delega a geracao a API.
export function GeneratorQuina() {
  const { state } = useAppState();
  // Preferencias persistidas localmente sao carregadas uma unica vez na montagem.
  const [storedPreferences] = useState(() => loadQuinaPreferences());
  ...
  const [result, setResult] = useState<GenerateQuinaGamesResponse | null>(null);
  ...
  // Preco da aposta simples vem da modalidade (fonte de verdade); 3.0 e o fallback da Quina.
  ...
  useEffect(() => {
    ...
    client
      .getJson<LatestContestResponse>("/concursos/quina/ultimo")
      .then((contest) => {
        if (active) {
      ...
      .catch(() => undefined);
    client
      .getJson<FilterStatisticsResponse>("/estatisticas/quina/filtros")
      .then((statistics) => {
        if (active) {
    ...
    client
      ...
      .then((modes) => {
        ...
        const quina = modes.find((mode) => mode.codigo === "quina");
        if (quina?.valorApostaSimples != null) {
          setBetPrice(quina.valorApostaSimples);
        }
      })
      .catch(() => undefined);
    return () => {
  ...
  useEffect(() => {
    saveQuinaPreferences({ choices, activeFilters, count, selection });
  ...
  async function generate() {
    ...
    try {
      ...
      const response = await client.postJson<GenerateGamesRequest, GenerateQuinaGamesResponse>(
        "/gerador/quina/gerar",
        request
      );
  ...
  return (
    <section className="page gen-page">
      <Link to="/dashboard/quina" className="gen-back-link">
      ...
      <section className="gen-filters" aria-label="Filtros matemáticos">
        <div className="gen-filters-head">
          <div>
            <h2>Filtros matemáticos · Quina</h2>
      ...
      <div className="gen-row">
        <section className="gen-numbers-card" aria-label="Dezenas incluídas ou excluídas">
          ...
          <div className="gen-numbers-body">
            <div className="gen-board gen-board--quina">
        ...
        <section className="gen-previous-card" aria-label="Concurso anterior">
          <div className="gen-card-head">
            ...
            <span className="gen-card-hint">
              repetição média: {repetitionAverage !== null ? repetitionAverage.toFixed(1).replace(".", ",") : "0,3"}
            </span>
          </div>
        ...
        {scriptVisible && games.length > 0 && (
          <div className="gen-script-box">
            <div className="gen-script-head">
              <span>
                Abra o volante da Quina no site da Caixa, pressione F12 → Console e cole este código para preencher
