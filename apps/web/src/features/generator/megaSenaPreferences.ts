// Persistencia local das preferencias do gerador da Mega-Sena (filtros, quantidade e dezenas marcadas).
import {
  ActiveFilterState,
  BOARD_SIZE,
  ChoiceState,
...
export const MEGA_SENA_PREFERENCES_KEY = "lotoanalytics.gerador.megasena.filtros";
...
export function saveMegaSenaPreferences(preferences: GeneratorPreferences, storage: Storage = localStorage): void {
  try {
    storage.setItem(MEGA_SENA_PREFERENCES_KEY, JSON.stringify(preferences));
  } catch {
...
export function loadMegaSenaPreferences(storage: Storage = localStorage): GeneratorPreferences | null {
  let raw: string | null;
  try {
    raw = storage.getItem(MEGA_SENA_PREFERENCES_KEY);
  } catch {
    return null;
  }
