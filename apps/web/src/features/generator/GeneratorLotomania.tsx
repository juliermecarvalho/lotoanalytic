...
import {
  ...
  GenerateLotomaniaGamesResponse,
  LatestContestResponse,
  LotteryModeResponse
} from "../../lib/apiClient";
import { buildGamesCsv, buildGamesScript } from "./gameExport";
import { loadLotomaniaPreferences, saveLotomaniaPreferences } from "./lotomaniaPreferences";
...
// Tela de geracao de apostas da Lotomania: monta os filtros estatisticos e delega a geracao a API.
// A aposta e sempre de 50 dezenas (regra oficial), por isso nao ha seletor de dezenas por jogo.
export function GeneratorLotomania() {
  const { state } = useAppState();
  // Preferencias persistidas localmente sao carregadas uma unica vez na montagem.
  const [storedPreferences] = useState(() => loadLotomaniaPreferences());
  ...
  const [count, setCount] = useState(() => storedPreferences?.count ?? 5);
  const [result, setResult] = useState<GenerateLotomaniaGamesResponse | null>(null);
  ...
  // Preco da aposta simples vem da modalidade (fonte de verdade); 3.0 e o fallback da Lotomania.
  const [betPrice, setBetPrice] = useState(3);
  const requestSequence = useRef(0);
  const selectionGenerationReady = useRef(false);

  // Carrega o concurso mais recente para exibir o sorteio anterior e o total de concursos.
  useEffect(() => {
    let active = true;
    const client = new ApiClient({ baseUrl: state.apiBaseUrl });
    client
      .getJson<LatestContestResponse>("/concursos/lotomania/ultimo")
      .then((contest) => {
        if (active) {
    ...
    client
      ...
      .then((modes) => {
        ...
        const lotomania = modes.find((mode) => mode.codigo === "lotomania");
        if (lotomania?.valorApostaSimples != null) {
          setBetPrice(lotomania.valorApostaSimples);
        }
      })
      .catch(() => undefined);
    return () => {
  ...
  // Para a Lotomania as estatisticas exibidas vem sempre da calibracao de referencia (aposta de 50
  // dezenas); por isso nao passamos as distribuicoes do banco, que descrevem sorteios de 20 dezenas.
  const liveStats = useMemo(() => buildLiveFilterStats(null), []);
  const contestLabel = latestContest ? formatInt(latestContest.numeroConcurso) : "—";
  const calibrationLabel = latestContest ? formatInt(latestContest.totalConcursos) : "milhares de";
  ...
  useEffect(() => {
    saveLotomaniaPreferences({ choices, activeFilters, count, selection });
  ...
  // Envia os filtros da tela para a API gerar as apostas no backend.
  async function generate() {
    ...
    try {
      ...
      const response = await client.postJson<GenerateGamesRequest, GenerateLotomaniaGamesResponse>(
  ...
  // Fixa as 5 primeiras dezenas do sorteio anterior como ponto de partida da estratégia.
  function suggestFixed() {
    const next: SelectionState = {};
    previousDraw.slice(0, 5).forEach((value) => {
      next[value] = "include";
    });
    setSelection(next);
  }
  ...
  let alertText = `Aposta fixa de ${GAME_SIZE} dezenas: ${BOARD_SIZE - excluded.length - included.length} dezenas disponíveis para completar cada cartão.`;
  ...
  if (tooManyIncluded || poolTooSmall) {
    ...
    alertText = `Só ${games.length} de ${count} apostas passaram nos filtros. Afrouxe um filtro ou reduza as fixas.`;
    alertTone = "warn";
  } else if (included.length >= 1) {
    alertText = `${included.length} ${included.length === 1 ? "dezena fixa" : "dezenas fixas"} em todos os cartões. As demais completam as ${GAME_SIZE} pelos filtros.`;
    alertTone = "ok";
  }

  const summaryText = generating
    ? "Gerando apostas no servidor…"
    : error
      ? error
      : result
        ? `${games.length} apostas válidas · ${formatInt(result.combinacoesTestadas)} combinações testadas${stale ? " · filtros alterados, regere" : ""}`
        : "clique em Gerar apostas para consultar o servidor";
  ...
  const note = selectedItem
    ? `Todas as apostas desta geração usarão: ${selectedItem.label}.`
    : "Nenhuma opção escolhida: este filtro não restringe a geração.";
  const seal = selectedItem ? `cobre ${formatPercent(coverage)} da referência` : "sem restrição";

  return (
    <section className="page gen-page">
      <Link to="/dashboard/lotomania" className="gen-back-link">
      ...
      <section className="gen-filters" aria-label="Filtros matemáticos">
        <div className="gen-filters-head">
          <div>
            <h2>Filtros matemáticos · Lotomania</h2>
            <p>
              A aposta tem sempre {GAME_SIZE} dezenas de 00 a 99. Cada filtro é calibrado sobre {calibrationLabel} sorteios reais. Selecione um para ver a estatística de referência.
            </p>
          </div>
        </div>
        ...
        <div className="gen-filters-body">
          <div className="gen-filter-list">
            {FILTER_ORDER.map((key) => {
              ...
              const detail = chosen
                ? `${chosen.label} · ${chosen.percent.toFixed(0)}% da referência`
          ...
          <div className="gen-stats">
            ...
            <div className="gen-stats-instruction">
              <span>Escolha uma opção — vale para todas as apostas desta geração</span>
        ...
        <div className="gen-weights">
          <div className="gen-weights-head">
            ...
            <span>como as apostas se dividem quando um filtro não é travado em uma única opção</span>
          </div>
      ...
      <div className="gen-row">
        <section className="gen-numbers-card" aria-label="Dezenas incluídas ou excluídas">
          ...
          <div className="gen-numbers-body">
            <div className="gen-board gen-board--lotomania">
              {Array.from({ length: BOARD_SIZE }, (_, index) => index + FIRST_NUMBER).map((value) => {
        ...
        <section className="gen-previous-card" aria-label="Sorteio anterior">
          <div className="gen-card-head">
            <h2>Sorteio anterior · {contestLabel}</h2>
            <span className="gen-card-hint">20 dezenas sorteadas</span>
          </div>
          ...
          <button type="button" className="gen-btn-suggest" onClick={suggestFixed}>
            Sugerir 5 fixas do anterior
          </button>
        </section>
      </div>

      <section className="gen-games" aria-label="Apostas geradas">
        <div className="gen-games-head">
          ...
          <div className="gen-games-controls">
            ...
            <button type="button" className="gen-btn-regen" onClick={generate} disabled={generating}>
              ...
              {generating ? "Gerando…" : "Gerar apostas"}
            </button>
          </div>
        </div>
        ...
        <div className="gen-games-list">
          {games.map((game, index) => {
            const sequenceOk = game.maiorSequencia <= 8;
            return (
              <div className="gen-game" key={game.dezenas.join("-")}>
                <div className="gen-game-head">
                  <span className="gen-game-title">Aposta {pad2(index + 1)}</span>
          ...
          {result && games.length === 0 && !generating && (
            <div className="gen-empty">
              <strong>Nenhuma aposta passou nos filtros</strong>
          ...
          {!result && !generating && (
            <div className="gen-empty">
              <strong className="gen-empty-neutral">Nenhuma aposta gerada ainda</strong>
              <span>Ajuste os filtros e clique em Gerar apostas — a geração roda no servidor com o seu plano.</span>
            </div>
          )}
        </div>
        ...
        <div className="gen-games-foot">
          <span className="gen-cost">
            ...
            · cada aposta da Lotomania tem 50 dezenas e custa R$ {formatCurrency(betPrice)}.
          </span>
          <div className="gen-games-foot-actions">
            <button
              ...
              onClick={() => downloadTextFile("apostas.csv", buildGamesCsv(games), "text/csv;charset=utf-8")}
            >
              Exportar CSV
            </button>
            <button
        ...
        {scriptVisible && games.length > 0 && (
          <div className="gen-script-box">
            <div className="gen-script-head">
              <span>
                Abra o volante da Lotomania no site da Caixa, pressione F12 → Console e cole este código para preencher
                as apostas automaticamente.
              </span>
...
function describeGenerationError(error: unknown): string {
  ...
  if (message.includes("401")) {
    return "Entre com Keycloak para gerar apostas.";
  }
