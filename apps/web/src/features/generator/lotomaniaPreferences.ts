// Persistencia local das preferencias do gerador da Lotomania (filtros, quantidade e dezenas marcadas).
import {
  ActiveFilterState,
  ChoiceState,
...
export const LOTOMANIA_PREFERENCES_KEY = "lotoanalytics.gerador.lotomania.filtros";
...
export function saveLotomaniaPreferences(preferences: GeneratorPreferences, storage: Storage = localStorage): void {
  try {
    storage.setItem(LOTOMANIA_PREFERENCES_KEY, JSON.stringify(preferences));
  } catch {
...
export function loadLotomaniaPreferences(storage: Storage = localStorage): GeneratorPreferences | null {
  let raw: string | null;
  try {
    raw = storage.getItem(LOTOMANIA_PREFERENCES_KEY);
  } catch {
    return null;
  }
...
// Aceita apenas dezenas validas do volante 00-99 com estado incluir/excluir.
function sanitizeSelection(value: unknown): GeneratorSelection {
  ...
  for (const [key, state] of Object.entries(source)) {
    ...
    if (
      ...
      numberValue >= FIRST_NUMBER &&
      numberValue <= LAST_NUMBER &&
