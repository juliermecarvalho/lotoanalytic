...
import {
  ...
  GenerateMaisMilionariaGamesResponse,
  LatestContestResponse,
  LotteryModeResponse
} from "../../lib/apiClient";
import { buildTrevoGamesCsv, buildTrevoGamesScript } from "./gameExport";
import { loadMaisMilionariaPreferences, saveMaisMilionariaPreferences } from "./maisMilionariaPreferences";
...
// Tela de geracao de jogos da +Milionaria: monta os filtros estatisticos, sorteia os trevos e delega a geracao a API.
export function GeneratorMaisMilionaria() {
  const { state } = useAppState();
  // Preferencias persistidas localmente sao carregadas uma unica vez na montagem.
  const [storedPreferences] = useState(() => loadMaisMilionariaPreferences());
  ...
  // Quantidade de trevos por jogo (2 a 6); a aposta minima da +Milionaria usa 2 trevos.
  const [trevosCount, setTrevosCount] = useState(() => storedPreferences?.trevosCount ?? MIN_TREVOS);
  const [result, setResult] = useState<GenerateMaisMilionariaGamesResponse | null>(null);
  ...
  // Preco da aposta simples vem da modalidade (fonte de verdade); 6.0 e o fallback da +Milionaria.
  ...
  useEffect(() => {
    ...
    client
      .getJson<LatestContestResponse>("/concursos/maismilionaria/ultimo")
      .then((contest) => {
        if (active) {
      ...
      .catch(() => undefined);
    client
      .getJson<FilterStatisticsResponse>("/estatisticas/maismilionaria/filtros")
      .then((statistics) => {
        if (active) {
    ...
    client
      ...
      .then((modes) => {
        ...
        const maisMilionaria = modes.find((mode) => mode.codigo === "maismilionaria");
        if (maisMilionaria?.valorApostaSimples != null) {
          setBetPrice(maisMilionaria.valorApostaSimples);
        }
      })
      .catch(() => undefined);
    return () => {
  ...
  // Marca o ultimo resultado como desatualizado quando filtros, dezenas ou trevos mudam.
  useEffect(() => {
    setStale(true);
  }, [selection, choices, activeFilters, count, trevosCount]);

  // Persiste localmente cada mudanca de filtros, quantidade, trevos ou dezenas marcadas.
  useEffect(() => {
    saveMaisMilionariaPreferences({ choices, activeFilters, count, trevosCount, selection });
  }, [choices, activeFilters, count, trevosCount, selection]);
  ...
  async function generate() {
    ...
    try {
      ...
      const response = await client.postJson<GenerateGamesRequest, GenerateMaisMilionariaGamesResponse>(
  ...
  // Ajusta a quantidade de trevos por jogo dentro da faixa oficial (2 a 6).
  function changeTrevosCount(event: ChangeEvent<HTMLSelectElement>) {
    setTrevosCount(Math.min(MAX_TREVOS, Math.max(MIN_TREVOS, parseInt(event.target.value, 10) || MIN_TREVOS)));
  }
  ...
  return (
    <section className="page gen-page">
      <Link to="/dashboard/mais-milionaria" className="gen-back-link">
      ...
      <section className="gen-filters" aria-label="Filtros matemáticos">
        <div className="gen-filters-head">
          <div>
            <h2>Filtros matemáticos · +Milionária</h2>
      ...
      <div className="gen-row">
        <section className="gen-numbers-card" aria-label="Dezenas incluídas ou excluídas">
          ...
          <div className="gen-numbers-body">
            <div className="gen-board gen-board--mais-milionaria">
      ...
      <section className="gen-games" aria-label="Jogos gerados">
        <div className="gen-games-head">
          ...
          <div className="gen-games-controls">
            ...
            <label>
              Trevos por jogo
              <select value={trevosCount} onChange={changeTrevosCount}>
                {Array.from({ length: MAX_TREVOS - MIN_TREVOS + 1 }, (_, index) => MIN_TREVOS + index).map((value) => (
          ...
          {games.map((game, index) => {
            ...
            return (
              <div className="gen-game" key={game.dezenas.join("-")}>
                ...
                <div className="gen-game-trevos" aria-label="Trevos do jogo">
                  <span className="gen-trevos-label">Trevos</span>
                  {game.trevos.map((trevo) => (
                    <span className="gen-chip gen-chip--trevo" key={`trevo-${trevo}`}>
                      {trevo}
                    </span>
                  ))}
                </div>
        ...
        <div className="gen-games-foot">
          ...
          <div className="gen-games-foot-actions">
            <button
              ...
              onClick={() => downloadTextFile("jogos.csv", buildTrevoGamesCsv(games), "text/csv;charset=utf-8")}
            >
              Exportar CSV
            </button>
            <button
        ...
        {scriptVisible && games.length > 0 && (
          <div className="gen-script-box">
            <div className="gen-script-head">
              <span>
                Abra o volante da +Milionária no site da Caixa, pressione F12 → Console e cole este código para preencher
                as dezenas e os trevos automaticamente.
              </span>
              <button type="button" className="gen-btn-regen" onClick={() => copyScript(buildTrevoGamesScript(games))}>
                {scriptCopied ? "Copiado!" : "Copiar código"}
              </button>
            </div>
            <pre className="gen-script-output" aria-label="Código jogos.js">
              {buildTrevoGamesScript(games)}
            </pre>
          </div>
        )}
      </section>
