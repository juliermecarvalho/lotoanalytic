...
import {
  ...
  GenerateDuplaSenaGamesResponse,
  LatestContestResponse,
  LotteryModeResponse
} from "../../lib/apiClient";
import { buildGamesCsv, buildGamesScript } from "./gameExport";
import { loadDuplaSenaPreferences, saveDuplaSenaPreferences } from "./duplaSenaPreferences";
...
// Tela de geracao de jogos da Dupla Sena: monta os filtros estatisticos e delega a geracao a API.
export function GeneratorDuplaSena() {
  const { state } = useAppState();
  // Preferencias persistidas localmente sao carregadas uma unica vez na montagem.
  const [storedPreferences] = useState(() => loadDuplaSenaPreferences());
  ...
  const [result, setResult] = useState<GenerateDuplaSenaGamesResponse | null>(null);
  ...
  // Preco da aposta simples vem da modalidade (fonte de verdade); 3.0 e o fallback da Dupla Sena.
  ...
  useEffect(() => {
    ...
    client
      ...
      .then((modes) => {
        ...
        const duplaSena = modes.find((mode) => mode.codigo === "dupla_sena");
        if (duplaSena?.valorApostaSimples != null) {
          setBetPrice(duplaSena.valorApostaSimples);
        }
      })
      .catch(() => undefined);
    return () => {
  ...
  useEffect(() => {
    saveDuplaSenaPreferences({ choices, activeFilters, count, selection });
  ...
  async function generate() {
    ...
    try {
      ...
      const response = await client.postJson<GenerateGamesRequest, GenerateDuplaSenaGamesResponse>(
  ...
  return (
    <section className="page gen-page">
      <Link to="/dashboard/dupla-sena" className="gen-back-link">
      ...
      <section className="gen-filters" aria-label="Filtros matemáticos">
        <div className="gen-filters-head">
          <div>
            <h2>Filtros matemáticos · Dupla Sena</h2>
      ...
      <div className="gen-row">
        <section className="gen-numbers-card" aria-label="Dezenas incluídas ou excluídas">
          ...
          <div className="gen-numbers-body">
            <div className="gen-board gen-board--dupla-sena">
        ...
        {scriptVisible && games.length > 0 && (
          <div className="gen-script-box">
            <div className="gen-script-head">
              <span>
                Abra o volante da Dupla Sena no site da Caixa, pressione F12 → Console e cole este código para preencher
