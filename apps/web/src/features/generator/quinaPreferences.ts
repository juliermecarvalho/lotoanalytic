// Persistencia local das preferencias do gerador da Quina (filtros, quantidade e dezenas marcadas).
import {
  ActiveFilterState,
  BOARD_SIZE,
  ChoiceState,
...
export const QUINA_PREFERENCES_KEY = "lotoanalytics.gerador.quina.filtros";
...
export function saveQuinaPreferences(preferences: GeneratorPreferences, storage: Storage = localStorage): void {
  try {
    storage.setItem(QUINA_PREFERENCES_KEY, JSON.stringify(preferences));
  } catch {
...
export function loadQuinaPreferences(storage: Storage = localStorage): GeneratorPreferences | null {
  let raw: string | null;
  try {
    raw = storage.getItem(QUINA_PREFERENCES_KEY);
  } catch {
    return null;
  }
