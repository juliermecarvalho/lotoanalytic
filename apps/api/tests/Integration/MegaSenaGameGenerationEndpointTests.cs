...
public sealed class MegaSenaGameGenerationEndpointTests
{
    [Fact]
    public async Task GenerateMegaSenaGamesReturnsGamesThatMatchFilters()
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<MegaSenaGameGenerationResponse>(TestContext.Current.CancellationToken);
        ...
        foreach (var game in result.Jogos)
        {
            ...
            game.Dezenas.ShouldNotContain("60");
            game.QuantidadePares.ShouldBe(3);
            game.QuantidadeImpares.ShouldBe(3);
            game.SomaDezenas.ShouldBeInRange(60, 220);
    ...
    [Fact]
    public async Task GenerateMegaSenaGamesPersistsGenerationHistory()
    {
        await using var postgres = await StartPostgresAsync();
        await using var factory = CreateAuthenticatedFactory(postgres.GetConnectionString());
        using var client = factory.CreateClient();
    ...
    // Inicia um PostgreSQL isolado para validar migrations e o fluxo de geracao da Mega-Sena.
    ...
    private sealed record MegaSenaGameGenerationResponse(
        ...
        : AuthenticationHandler<AuthenticationSchemeOptions>(options, logger, encoder)
    {
        ...
        protected override Task<AuthenticateResult> HandleAuthenticateAsync()
        {
            var claims = new[]
            {
                ...
                new Claim("preferred_username", "gerador.megasena"),
                new Claim(ClaimTypes.Email, "gerador.megasena@lotoanalytics.local"),
