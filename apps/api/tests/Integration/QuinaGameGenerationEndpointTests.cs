...
public sealed class QuinaGameGenerationEndpointTests
{
    [Fact]
    public async Task GenerateQuinaGamesReturnsGamesThatMatchFilters()
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<QuinaGameGenerationResponse>(TestContext.Current.CancellationToken);
        ...
        {
            game.Dezenas.Count.ShouldBe(5);
            game.Dezenas.ShouldContain("01");
            game.Dezenas.ShouldContain("02");
            game.Dezenas.ShouldNotContain("80");
            game.QuantidadePares.ShouldBe(2);
            game.QuantidadeImpares.ShouldBe(3);
            game.SomaDezenas.ShouldBeInRange(60, 280);
    ...
    [Fact]
    public async Task GenerateQuinaGamesPersistsGenerationHistory()
    {
        await using var postgres = await StartPostgresAsync();
        await using var factory = CreateAuthenticatedFactory(postgres.GetConnectionString());
        using var client = factory.CreateClient();
        ...
        history.Geracoes[0].DezenasPorJogo.ShouldBe(5);
    ...
    // Inicia um PostgreSQL isolado para validar migrations e o fluxo de geracao da Quina.
    ...
    private sealed record QuinaGameGenerationResponse(
        ...
        : AuthenticationHandler<AuthenticationSchemeOptions>(options, logger, encoder)
    {
        ...
        protected override Task<AuthenticateResult> HandleAuthenticateAsync()
        {
            var claims = new[]
            {
                ...
                new Claim("preferred_username", "gerador.quina"),
                new Claim(ClaimTypes.Email, "gerador.quina@lotoanalytics.local"),
