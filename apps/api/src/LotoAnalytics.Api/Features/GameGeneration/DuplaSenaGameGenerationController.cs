...
[ApiController]
[Route("gerador/dupla-sena")]
public sealed class DuplaSenaGameGenerationController(IServiceProvider serviceProvider) : ControllerBase
{
    // Gera jogos da Dupla Sena usando os filtros estatisticos informados no corpo da requisicao.
    [AllowAnonymous]
    [HttpPost("gerar")]
    public async Task<ActionResult<DuplaSenaGameGenerationResponse>> Generate(
        [FromBody] GenerateDuplaSenaGamesRequest request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var result = DuplaSenaGameGenerator.Generate(new DuplaSenaGameGenerationRequest
            {
            ...
            return Ok(new DuplaSenaGameGenerationResponse(
    ...
    // Carrega as chaves dos sorteios ja registrados da Dupla Sena para gerar apenas jogos ineditos.
    private async Task<IReadOnlySet<string>> LoadDrawnGameKeysAsync(CancellationToken cancellationToken)
    {
        ...
        var drawnNumbers = await dbContext.ContestNumbers
            ...
            .Where(number => number.NumberType == "principal" &&
                number.Contest!.LotteryMode!.Code == "dupla_sena" &&
        ...
        return drawnNumbers
            ...
            .Select(group => DuplaSenaGameGenerator.FormatGameKey(group.Select(number => number.NumericValue!.Value).ToArray()))
            .ToHashSet(StringComparer.Ordinal);
    }

    // Converte o jogo gerado para o contrato HTTP em PT-BR.
    private static GeneratedDuplaSenaGameResponse MapGeneratedGame(GeneratedDuplaSenaGame game)
    {
        return new GeneratedDuplaSenaGameResponse(
...
public sealed record GenerateDuplaSenaGamesRequest
{
...
public sealed record DuplaSenaGameGenerationResponse(
    [property: JsonPropertyName("jogos")] IReadOnlyList<GeneratedDuplaSenaGameResponse> Games,
    [property: JsonPropertyName("combinacoesTestadas")] int AttemptCount);

public sealed record GeneratedDuplaSenaGameResponse(
