...
export function GeneratorPage() {
  ...
  const [count, setCount] = useState(() => storedPreferences?.count ?? 6);
  const [result, setResult] = useState<GenerateGamesResponse | null>(null);
  ...
  useEffect(() => {
    ...
    client
      .getJson<LatestContestResponse>("/concursos/lotofacil/ultimo")
      .then((contest) => {
        if (active) {
      ...
      .catch(() => undefined);
    client
      .getJson<FilterStatisticsResponse>("/estatisticas/lotofacil/filtros")
      .then((statistics) => {
        if (active) {
  ...
  useEffect(() => {
    saveGeneratorPreferences({ choices, activeFilters, count, selection });
  ...
  async function generate() {
    ...
    try {
      ...
      const response = await client.postJson<GenerateGamesRequest, GenerateGamesResponse>(
  ...
  // Fixa as 9 primeiras dezenas do concurso anterior, conforme a estratégia do Grupo A.
  function suggestFixed() {
    const next: SelectionState = {};
    previousDraw.slice(0, 9).forEach((value) => {
      next[value] = "include";
    });
    setSelection(next);
  }
  ...
  if (tooManyIncluded || poolTooSmall) {
    alertText = tooManyIncluded
      ? `Mais de 15 dezenas incluídas — remova ${included.length - GAME_SIZE}.`
      : "Excluídas demais: sobram menos de 15 dezenas no volante.";
    alertTone = "danger";
    ...
    alertTone = "warn";
  } else if (included.length >= 9) {
    alertText = `${included.length} dezenas fixas em todos os cartões (Grupo A). As demais rodam pelos filtros.`;
    alertTone = "ok";
  }
  ...
  return (
    <section className="page gen-page">
      <header className="gen-header">
        <div>
          <h1>Geração de jogos</h1>
          <p>
            Concurso base {contestLabel}
            {latestContest?.dataApuracao ? ` · ${formatDate(latestContest.dataApuracao)}` : ""} · 3.411 sorteios na base
          </p>
        </div>
        <div className="gen-header-actions">
          <button type="button" className="gen-btn-secondary" onClick={() => saveGeneratorPreferences({ choices, activeFilters, count, selection })}>
            Salvar preset
          </button>
          <button type="button" className="gen-btn-primary" onClick={generate} disabled={generating}>
          ...
          <div className="gen-numbers-body">
            <div className="gen-board">
              {Array.from({ length: BOARD_SIZE }, (_, index) => index + 1).map((value) => {
                ...
                const modifier = state === "include" ? " gen-cell--include" : state === "exclude" ? " gen-cell--exclude" : "";
                return (
        ...
        <section className="gen-previous-card" aria-label="Concurso anterior">
          <div className="gen-card-head">
            ...
            <span className="gen-card-hint">
              repetição média: {repetitionAverage !== null ? repetitionAverage.toFixed(1).replace(".", ",") : "9,0"}
            </span>
          </div>
      ...
      <section className="gen-games" aria-label="Jogos gerados">
        ...
        <div className="gen-games-list">
          {games.map((game, index) => {
            const frameOk = game.quantidadeMoldura >= 8 && game.quantidadeMoldura <= 11;
            const sequenceOk = game.maiorSequencia <= 5;
            return (
              <div className="gen-game" key={game.dezenas.join("-")}>
                <div className="gen-game-head">
                  ...
                  <div className="gen-game-badges">
                    <span className={`gen-badge gen-badge--${frameOk ? "ok" : "warn"}`}>
                      moldura {game.quantidadeMoldura}
                    </span>
          ...
          {result && games.length === 0 && !generating && (
            <div className="gen-empty">
              ...
              <span>Afrouxe uma opção (ex.: soma, primos ou moldura) ou reduza as dezenas fixas.</span>
            </div>
          )}
        ...
        {scriptVisible && games.length > 0 && (
          <div className="gen-script-box">
            <div className="gen-script-head">
              <span>
                Abra o volante da Lotofácil no site da Caixa, pressione F12 → Console e cole este código para preencher
      ...
      <aside className="gen-ad-rail" aria-label="Publicidade e assinatura">
        ...
        <section className="gen-no-ads">
          <strong>Plano sem anúncios</strong>
          <p>Use todos os filtros com uma experiência mais limpa.</p>
          <button type="button" className="gen-btn-primary">Assinar</button>
        </section>
      </aside>
    </section>
  );
