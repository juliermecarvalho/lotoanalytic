...
describe("LotoAnalytics web", () => {
  ...
  it("generates games through the backend with the statistical filters", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Geração de jogos" })).toBeInTheDocument();
    ...
    // O concurso mais recente vem da API e alimenta o cabecalho e o card do concurso anterior.
    expect(await screen.findByRole("heading", { name: "Concurso anterior · 3.411" })).toBeInTheDocument();
    expect(screen.getByText(/3\.411 sorteios na base/)).toBeInTheDocument();
    ...
    // As ações principais ficam disponíveis no cabeçalho e no bloco de jogos.
    expect(screen.getByRole("button", { name: "Salvar preset" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Regerar" })).toBeInTheDocument();
    expect(screen.getAllByRole("button", { name: "Gerar jogos" })).toHaveLength(1);
  ...
  it("restores generator filters from localStorage and clears them on demand", async () => {
    ...
    expect(await screen.findByLabelText("Quantidade")).toHaveValue(6);
